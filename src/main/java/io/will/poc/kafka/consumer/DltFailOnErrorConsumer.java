...
@Component
public class DltFailOnErrorConsumer {
    private final Logger LOGGER = LoggerFactory.getLogger(DltFailOnErrorConsumer.class);

    @RetryableTopic(attempts = "1", kafkaTemplate = "retryableKafkaTemplate", dltStrategy = DltStrategy.FAIL_ON_ERROR)
    @KafkaListener(topics = {TOPIC_RETRYABLE_FAIL_ON_ERROR}, containerFactory = "greetingKafkaListenerContainerFactory")
